...
<h1>Data Machine Cost </h1>
	...
	<? $s=1;
		...
		<td><?=$datatampil['hrg']?></td>
		<td><?=$datatampil['ket']?></td>
		<td>
		<a href="index.php?h=<?=$_GET['h']?>&link=frm_addmesinprc.php&usr=<?=$datatampil['idmesin']?>">Edit</a> 
	...
	<div class="tablefooter clearfix">
						<div class="pagination">
							<a href="index.php?h=<?=$_GET['h']?>&link=df_mesinprc.php&pg=<?=$page-1?>">Prev</a>
							<?  
							//$urlnya = $_SERVER['REQUEST_URL']."&pg=1";
							for($z=1;$z<=$all;$z++){ 
							 if ((($page >= $z - 3) && ($page <= $z + 3)))
								{
							?><a href="index.php?h=<?=$_GET['h']?>&link=df_mesinprc.php&pg=<?=$z?>"><?=$z?></a><? 
								}
								} ?>
							<a href="index.php?h=<?=$_GET['h']?>&link=df_mesinprc.php&pg=<?=$page+1?>">Next</a>
						</div>
	</div>
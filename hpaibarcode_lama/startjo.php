...
<table width='100%'>
			...
			<td> <input type="hidden"  <?=$read?> value="<?=$dtrun[nojo]?>"  id="txkode" name="txkode" class="small required" size="10" />  </td><td> </td> </tr>
			...
			<tr> <td> <b> Quantity  </b> </td><td> :  </td><td> <?=$dtrun[qty]?> </td> 
			<td> <b>NG Qty </b> </td><td> :  </td><td> <input type="text"  <?=$read?> value="0"  id="txng" name="txng" class="small required" size="10" /> </td> </tr>
</table>
<div class="buttons">
...
<br />
<table width='100%'>
	<tr><th>Code Sub processes</th><th>Code processes</th><th>Estimasi Time</th><th>&nbsp;</th></tr>
	<? 	$r=1;
...
WHERE nojo='$dtjo[nojo]' ");
		...
		<a href="index.php?h=<?=$_GET['h']?>&link=startjo2.php&nojo=<?=$dtjo[nojo]?>&noproses=<?=$datatetail2['idproses']?>&tqty=<?=$dtjo[qty]?>"> Run </a> </td> 
		<?}else{ ?>
		Done !
		<?} ?>
		</tr>

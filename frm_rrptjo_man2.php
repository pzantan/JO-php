...
if ($_POST['txcust']<>"") { $tambahline = "and t.pelaksana like '%$_POST[txcust]%' ";}
...
Tangal : <?=$tanggal1?> s/d <?=$tanggal2?> <br />
					<table id="table2" border="1" cellpadding="0" cellspacing="0" class="gtable detailtable">
						...
						<? 	$r=1; $totreal=0;
						$hasildetail2=mysql_query("SELECT t.*, m.`nmmesin`,  p.`nmproses`, MINUTE(TIMEDIFF(t.`endtime`,t.`starttime` )) AS realtime, u.`usr_nama` FROM tr_jodetail t
LEFT JOIN m_mesin m ON t.`idmesin`=m.`idmesin`
LEFT JOIN mproses p ON t.`idproses`=p.`idproses`
LEFT JOIN m_user u ON t.`pelaksana` = u.`nik`
where	

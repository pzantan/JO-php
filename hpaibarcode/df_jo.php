...
<h1>Data JO </h1>
	...
	<? $s=1;
			$hasilqw=mysql_query("SELECT t.*, p.`nmproses`, c.`nm_cust`,t.sts, t.qty FROM tr_jo t
LEFT JOIN mproses p ON t.`iproses`= p.`idproses`
LEFT JOIN m_cust c ON t.`idcust`=c.`id_cust` where t.sts<>'bukan apapa' $tambhan $tmb2 ORDER BY t.tgljo desc limit $pages,$per  ");

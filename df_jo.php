...
if ($_GET['pg']==""){
	...
	$qcount=mysql_query("SELECT COUNT(*) FROM tr_jo t where t.sts<>'bukan apapa' $tambhan $tmb2 ");
...
<h1>Data JO (<a href="index.php?h=<?=$_GET['h']?>&link=frm_addjo.php">Add +</a> )</h1>
	<form action="index.php"> 
					<fieldset>
						<legend>Filter JO</legend>
						<dl class="inline">
						<dt><label for="name">JO No</label></dt>
							<dd>
	...
	<? $s=1;
			$hasilqw=mysql_query("SELECT t.*, p.`nmproses`, c.`nm_cust`,t.sts, t.qty, d.`idproses` FROM tr_jo t
LEFT JOIN m_cust c ON t.`idcust`=c.`id_cust` 
LEFT JOIN tr_jodetail d ON t.nojo=d.`nojo` AND d.`sts`='r'
LEFT JOIN mproses p ON d.`idproses`= p.`idproses` where t.sts<>'bukan apapa' $tambhan $tmb2 ORDER BY t.tgljo desc limit $pages,$per  ");
			while ($datatampil=mysql_fetch_array($hasilqw)){ 
			if ($datatampil['sts']=="i"){$status="Idle"; $editer = "<a href=index.php?h=$_GET[h]&link=frm_addjo_edit.php&idjo=$datatampil[nojo]>$datatampil[nojo]</a>";}
			if ($datatampil['sts']=="r"){$status="<a href=index.php?h=$_GET[h]&link=frm_detailjo.php&idjo=$datatampil[nojo]>Running...</a>"; $editer = $datatampil['nojo'] ;}
